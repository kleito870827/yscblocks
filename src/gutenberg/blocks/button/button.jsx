...
class ButtonSingleBlock extends Component {
    render() {
        ...
        className = classnames(
            'ysc-button',
            size ? `ysc-button-${ size }` : '',
            hideText ? 'ysc-button-icon-only' : '',
            className
        );

        // focus outline
        if ( focusOutlineWeight && focusOutlineColor ) {
            className = classnames( className, 'ysc-button-with-outline' );
        }
            ...
            <Fragment>
                            ...
                            tabs={ colorsTabs }>
                            {
                                ( tabData ) => {
                                    ...
                                    if ( 'focus' === tabData.name ) {
                                        return (
                                            <ApplyFilters name="ysc.editor.controls" attribute="focusOutlineColor" props={ this.props }>
                                                <ColorPicker
                                                    label={ __( 'Outline' ) }
                                    ...
                                    return (
                                        <Fragment>
                                            ...
                                            <ApplyFilters name="ysc.editor.controls" attribute={ isHover ? 'hoverTextColor' : 'textColor' } props={ this.props }>
                                                <ColorPicker
                                                    label={ __( 'Text' ) }
                                            ...
                                            { borderWeight ? (
                                                <ApplyFilters name="ysc.editor.controls" attribute={ isHover ? 'hoverBorderColor' : 'borderColor' } props={ this.props }>
                                                    <ColorPicker
                                                        label={ __( 'Border' ) }
                ...
                <div>
                    <div className={ className }>
                        { icon && iconPosition === 'left' ? (
                            <div className="ysc-button-icon ysc-button-icon-left">
                                <IconPicker.Dropdown
                                    onChange={ ( value ) => setAttributes( { icon: value } ) }
                                    value={ icon }
                        ...
                        { icon && iconPosition === 'right' ? (
                            <span className="ysc-button-icon ysc-button-icon-right">
                ...
                { isSelected ? (
                    <URLInput
                        ...
                        autoFocus={ false }
                        className="ysc-component-url-input-float"
                    />
                ) : '' }
            </Fragment>
        );
...
export const name = 'ysc/button-single';
...
export const settings = {
    ...
    attributes: {
        ...
        text: {
            type: 'array',
            source: 'children',
            selector: '.ysc-button .ysc-button-text',
            default: 'Button',
        },
        hideText: {
            type: 'boolean',
    ...
    save: function( props ) {
        ...
        if ( ! hideText ) {
            result.push( <RichText.Content tagName="span" className="ysc-button-text" value={ text } key="button-text" /> );
        }
        ...
        if ( icon ) {
            if ( iconPosition === 'right' ) {
                result.push(
                    <span className="ysc-button-icon ysc-button-icon-right" key="button-icon">
                        <IconPicker.Render name={ icon } />
                    </span>
                );
            } else {
                result.unshift(
                    <span className="ysc-button-icon ysc-button-icon-left" key="button-icon">
                        <IconPicker.Render name={ icon } />
                    </span>
                );
            }
