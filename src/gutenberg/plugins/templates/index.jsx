...
class TemplatesModal extends Component {
    ...
    render() {
        ...
        return (
            <Modal
                className={ classnames( 'ysc-plugin-templates-modal ysc-plugin-templates-modal-hide-header', showLoadingSpinner ? 'ysc-plugin-templates-modal-loading' : '' ) }
                position="top"
                size="lg"
                onRequestClose={ () => {
                    onRequestClose();
            ...
            >
                <div className="components-modal__header">
                    ...
                    { showLoadingSpinner ? (
                        <div className="ysc-plugin-templates-modal-loading-spinner"><Spinner /></div>
                    ) : '' }
                    <button
                        type="button"
                ...
                { allTemplates && allTemplates.length ? (
                    <TabPanel
                        className="ysc-control-tabs ysc-component-modal-tab-panel"
                        tabs={ [
                    ...
                    >
                        {
                            ( tabData ) => {
                                ...
                                if ( 'pages' === tabType ||  'external-blocks' === tabType) {
                                ...
                                return (
                                    <Fragment>
                                        { currentTemplates === false ? (
                                            <div className="ysc-plugin-templates-spinner"><Spinner /></div>
                                        ) : '' }
                                            ...
                                            <div>
                                                { 'local' === tabType ? (
                                                    <Fragment>
                                                        ...
                                                        <a className="components-button is-button is-primary" href={ YSC.adminTemplatesUrl } target="_blank" rel="noopener noreferrer">{ __( 'Add Template' ) }</a>
                                                    </Fragment>
                                                ) : (
                                        ...
                                        { currentTemplates && currentTemplates.length ? (
                                            <Fragment>
                                                <div className="ysc-plugin-templates-categories-row">
                                                    <div className="ysc-plugin-templates-categories-select">
                                                        { this.printCategorySelect( tabType ) }</div>
                                                    <div className="ysc-plugin-templates-count">
                                                        <RawHTML>{ sprintf( __( 'Templates: %s' ), `<strong>${ currentTemplates.length }</strong>` ) }</RawHTML>
                                                    </div>
                                                </div>
                                                { this.state.error }
                                                <Masonry
                                                    className="ysc-plugin-templates-list"
                                                    elementType="ul"
                                                ...
                                                >
                                                    { currentTemplates.map( ( template ) => {
                                                        ...
                                                        return (
                                                            <li
                                                                className={ classnames( 'ysc-plugin-templates-list-item', withThumb ? '' : 'ysc-plugin-templates-list-item-no-thumb' ) }
                                                                key={ template.id }
                                                            >
                                                                <button
                                                                    onClick={ () => {
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="ysc-plugin-templates-list-item-title">{ template.title }</div>
                                                                </button>
                                                            </li>
                                                        );
                                                    } ) }
...
function checkMissingBlocks( data ) {
    ...
    const missingBlocksInfo = applyFilters( 'ysc.templates.missingBlocksInfo', {
        'nk/awb': {
    ...
    if ( Object.keys( missingBlocks ).length ) {
        Object.keys( missingBlocks ).forEach( ( blockName ) => {
            result.push(
                <div
                    className="ysc-alert ysc-templates-missing-block-alert"
                ...
                >
                    ...
                    { missingBlocksInfo[ blockName ] && missingBlocksInfo[ blockName ].info ? (
                        <div className="ysc-templates-missing-block-additional">
...
const TemplatesModalWithSelect = compose( [
    ...
    withSelect( ( select ) => {
        const templates = select( 'ysc/plugins/templates' ).getTemplates();
        ...
        return {
            ...
            getTemplateData( data, cb ) {
                ...
                apiFetch( {
                    path: `/ysc/v1/get_template_data/?id=${ data.id }&type=${ type }`,
                    method: 'GET',
                } ).then( ( result ) => {
                    cb( result );
                } );
