 ...
 * Class YSC_Rest
 */
class YSC_Rest extends WP_REST_Controller {
    /**
     * Namespace.
     *
     * @var string
     */
    protected $namespace = 'ysc/v';
    ...
    public function get_customizer() {
        $options = get_option( 'ysc_customizer_fields' );
    ...
    public function get_instagram_feed( WP_REST_Request $request ) {
        $cache_name = 'ysc_instagram_feed_cache';
    ...
    public function get_instagram_profile( WP_REST_Request $request ) {
        $cache_name = 'ysc_instagram_profile_cache';
        $cache_expiration = $request->get_param( 'cache_expiration' ) ? : 60 * 60 * 24; // 1 day in seconds.
        $access_token = $request->get_param( 'access_token' );
    ...
    public function get_twitter_profile( WP_REST_Request $request ) {
        $cache_name = 'ysc_twitter_profile_cache';
        $cache_expiration = $request->get_param( 'cache_expiration' ) ? : 60 * 60 * 24; // 1 day in seconds.
    ...
    public function get_twitter_feed( WP_REST_Request $request ) {
        $cache_name = 'ysc_twitter_feed_cache';
        $cache_expiration = $request->get_param( 'cache_expiration' ) ? : 60 * 60 * 24; // 1 day in seconds.
    ...
    public function get_templates() {
        $url = 'https://external-url/v1/get_library/';
        $templates = get_transient( 'ysc_remote_templates', false );
        ...
        if ( ! $templates ) {
            $requested_templates = wp_remote_get( add_query_arg( array(
                'ysc_version'     => '@@plugin_version',
                'ysc_pro'         => function_exists( 'ysc_pro' ),
                'ysc_pro_version' => function_exists( 'ysc_pro' ) ? ysc_pro()->$plugin_version : null,
            ), $url ) );
            ...
            if ( ! is_wp_error( $requested_templates ) ) {
                ...
                if ( $new_templates && isset( $new_templates['response'] ) && is_array( $new_templates['response'] ) ) {
                    ...
                    set_transient( 'ysc_remote_templates', $templates, WEEK_IN_SECONDS );
                }
            }
        }
        ...
        while ( $local_templates_query->have_posts() ) {
            ...
            $categories = array();
            $category_terms = get_the_terms( $db_template->ID, 'ysc_template_category' );
        ...
        foreach ( glob( get_template_directory() . '/ysc/templates/*/content.php' ) as $template ) {
        ...
        if ( get_stylesheet_directory() !== get_template_directory() ) {
            foreach ( glob( get_stylesheet_directory() . '/ysc/templates/*/content.php' ) as $template ) {
                $template_path = dirname( $template );
    ...
    public function get_template_data( WP_REST_Request $request ) {
        $url = 'https://external-url/v1/get_library_item/';
        $id = $request->get_param( 'id' );
        ...
        switch ( $type ) {
            case 'remote':
                $template_data = get_transient( 'ysc_template_' . $type . '_' . $id, false );
                ...
                if ( ! $template_data ) {
                    ...
                    if ( ! is_wp_error( $requested_template_data ) ) {
                        ...
                        if ( $new_template_data && isset( $new_template_data['response'] ) && is_array( $new_template_data['response'] ) ) {
                            ...
                            set_transient( 'ysc_template_' . $type . '_' . $id, $template_data, WEEK_IN_SECONDS );
                        }
                    }
                }
                break;
            case 'local':
                ...
                if ( $post && 'ysc_template' === $post->post_type ) {
                    $template_data = array(
                        'id'      => $post->ID,
                ...
                break;
            case 'theme':
                $template_content_file = get_stylesheet_directory() . '/ysc/templates/' . $id . '/content.php';

                if ( ! file_exists( $template_content_file ) ) {
                    $template_content_file = get_template_directory() . '/ysc/templates/' . $id . '/content.php';
                }
    ...
    public function get_custom_code() {
        $custom_code = get_option( 'ysc_custom_code', array() );
    ...
    public function update_custom_code( WP_REST_Request $request ) {
        ...
        if ( is_array( $new_code ) ) {
            $current_code = get_option( 'ysc_custom_code', array() );
            $updated = update_option( 'ysc_custom_code', array_merge( $current_code, $new_code ) );
        }
    ...
    public function update_google_maps_api_key( WP_REST_Request $request ) {
        $updated = update_option( 'ysc_google_maps_api_key', $request->get_param( 'key' ) );
    ...
    public function update_disabled_blocks( WP_REST_Request $request ) {
        ...
        if ( is_array( $new_disabled_blocks ) ) {
            $disabled_blocks = array_merge( get_option( 'ysc_disabled_blocks', array() ), $new_disabled_blocks );
            $result = array();
            ...
            $updated = update_option( 'ysc_disabled_blocks', $result );
        }
    ...
    public function update_settings( WP_REST_Request $request ) {
        ...
        if ( is_array( $new_settings ) ) {
            $current_settings = get_option( 'ysc_settings', array() );
            $updated = update_option( 'ysc_settings', array_merge( $current_settings, $new_settings ) );
        }
